...
function Rate = RS_1layer_Rate(H_est,Hall,SNRdB,weight,tolerance,M,c,d,Nt,N_user,Rth)
SNR = 10^(SNRdB/10);
%N_user * Nt
...
% Nt=2, N_user=4
nh1 = norm(H_est(1,:)); nh2 = norm(H_est(2,:)); nh3 = norm(H_est(3,:)); nh4 = norm(H_est(4,:));
nh = [nh1,nh2,nh3,nh4];
[~,nhidx1] = max(nh);

poweral = [0.1,0.1,0.1,0.1];
poweral(nhidx1) = 7*poweral(nhidx1);


p_p = sqrt(P_private.*poweral).*H_est'./(sum(abs(H_est).^2,2).^0.5)';
...
while true
    ...
    [WMMSE,p_p,p_c] = RS_1layer_CVX_Optimization(SNR,weight,Nt,N_user,u_opt_c_aveg,u_opt_p_aveg,e_c_aveg,e_p_aveg,f_c_aveg,f_p_aveg,v_c_aveg,v_p_aveg,w_c_aveg,w_p_aveg,Rth);
