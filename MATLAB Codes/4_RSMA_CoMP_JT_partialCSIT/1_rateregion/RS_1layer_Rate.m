...
function Rate = RS_1layer_Rate(H_est,Hall,SNRdB,weight,tolerance,M,c,d,Nt,N_user)
SNR = 10^(SNRdB/10);
%N_user * Nt
...
while true
    ...
    [WMMSE,p_p,p_c] = RS_1layer_CVX_Optimization(SNR,weight,Nt,N_user,u_opt_c_aveg,u_opt_p_aveg,e_c_aveg,e_p_aveg,f_c_aveg,f_p_aveg,v_c_aveg,v_p_aveg,w_c_aveg,w_p_aveg);
...
% Rate
Rate(1,:) = w_p_aveg(1) + max(min(w_c_aveg),0); %assume common stream is for user1 only
Rate(2,:) = w_p_aveg(2);
