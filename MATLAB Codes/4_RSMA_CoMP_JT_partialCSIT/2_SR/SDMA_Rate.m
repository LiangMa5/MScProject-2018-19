...
function Rate = SDMA_Rate(H_est,Hall,SNRdB,weight,tolerance,M,c,d,N_bs,N_user,Rth)
SNR = 10^(SNRdB/10);
%N_user * Nt
%% Step 1: Initialise precoder
P_private = SNR;

p_p0 = sqrt(P_private/N_user).*H_est'./(sum(abs(H_est).^2,2).^0.5)';

% %two user
% h1 = H_est(1,:);
% h2 = H_est(2,:);
% if norm(h1) >= norm(h2)
%     p_p0 = sqrt(P_private.*[0.9,0.1]).*H_est'./(sum(abs(H_est).^2,2).^0.5)';
% end
% 
% if norm(h1) < norm(h2)
%     p_p0 = sqrt(P_private.*[0.1,0.9]).*H_est'./(sum(abs(H_est).^2,2).^0.5)';
% end
...
p_p = sqrt(Power_bs)*p_p0./sqrt(P_bs);
...
while true
    ...
    [WMMSE,p_p] = SDMA_CVX_Optimization(SNR,weight,N_bs,N_user,u_opt_p_aveg,e_p_aveg,f_p_aveg,v_p_aveg,w_p_aveg,Rth);
